...
summary(source)
ggplot(source_long, aes(x = condition, y = accuracy, fill = condition)) + geom_boxplot(width=0.6) + geom_violin(trim=FALSE, width=0.8, alpha=0.5)

#Power Analysis
pwr.t.test(d = 0.2, sig.level = .05, power = 0.8, type = "two.sample")
pwr.t.test(d = 0.2, sig.level = .05, n = 500, type = "two.sample")
```
...
#Boxplot of NV-V and V-V
ggplot(H2_df, aes(x = condition, y = accuracy, fill = condition)) + geom_boxplot(width=0.6) + geom_violin(trim=FALSE, width=0.8, alpha=0.5) + scale_fill_manual(values=c("#3F3FFF","#82C1FF"))
#Perform paired t-test
